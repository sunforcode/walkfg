...
class RouteOverviewWidget extends StatelessWidget {
  ...
  List<Map<String, dynamic>> _getDisplayTags() {
    ...
    // 2. 特色标签（高优先级）
    final featureTags = route.featureTags.take(2);
  ...
  String _buildPracticalInfoText() {
    ...
    if (route.dailyPlans.isNotEmpty) {
      ...
      buffer.write('行程安排：从${route.startPoint}出发，');
      buffer.write('平均每天徒步${avgTimePerDay.toStringAsFixed(1)}小时，');

      if (route.dailyPlans.length > 1) {
        buffer.write('${route.dailyPlans.length}天行程最终在${route.endPoint}结束。');
      } else {
        buffer.write('当天在${route.endPoint}结束。');
      }
    }
    ...
    // 许可证信息（如果需要）
    if (route.facilities?.requiresPermit == true) {
      buffer.write('此路线需要提前申请相关许可证。');
    }
