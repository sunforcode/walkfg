...
## Purpose
统一的设计系统 (Design System)，提供标准化的 Design Token、主题配置和 UI 规范，确保应用界面风格一致。
...
应用 SHALL 为不同天气条件提供预定义的渐变色 Token，供天气卡片中洋溢变化使用:

**天气渐变色范例**:

| 天气条件 | 渐变颜色 | 设计意图 |
|---------|---------|--------|
| 晴天 | 橙色 (#FF9800 → #F57C00) | 温暖、阳光 |
| 多云 | 灰蓝色 (#90A4AE → #546E7A) | 中性、温和 |
| 下雨 | 蓝色 (#2196F3 → #1565C0) | 冷调、潮湿 |
| 下雪 | 浅蓝色 (#B3E5FC → #4FC3F7) | 清冷、纯洁 |
| 有雾 | 灰色 (#78909C → #455A64) | 模糊、暗沉 |
| 有风 | 青蓝色 (#009688 → #00796B) | 流动感、清爽 |
| 默认 | 蓝色 (#2196F3 → #1976D2) | 与主色调统一 |

**实现方式**:
- 提供方法 `getWeatherGradient(String? condition)` 处理条件判断
- 方法不挑剔地识别条件，返回对应的 `LinearGradient`
- 无法识别的条件返回默认蓝色渐变

#### Scenario: 天气卡片使用渐变色
- **WHEN** 开发者需要为天气卡片设置背景渐变
- **THEN** 调用 `AppColors.getWeatherGradient(weather.condition)` 获取渐变
- **THEN** 渐变自动对应天气条件，无需硬编码颜色值

#### Scenario: 无法识别的天气条件
- **WHEN** 天气条件为null或未知值
- **THEN** 返回默认的蓝色渐变 (#2196F3 → #1976D2)

---

### Requirement: Design Token - Trip 颜色调色板
应用 SHALL 为行程卡片提供统一的绿色系颜色调色板 Token，便于定义化处理:
